...
class DBManager:
    def __init__(self, db_name="taskflow.db"):        
        # Xác định đường dẫn lưu trữ an toàn trong thư mục AppData của người dùng
        ...
        # Kiểm tra xem file database đã tồn tại ở vị trí đó chưa
        if not os.path.exists(self.db_path):
            print(f"Database không tồn tại tại {self.db_path}. Đang sao chép từ bản mẫu...")
            # Nếu chưa, lấy đường dẫn đến file database "bản mẫu" đi kèm với file exe
            template_db_path = resource_path(db_name)
            # Sao chép file bản mẫu đến vị trí làm việc
            shutil.copy2(template_db_path, self.db_path)
            print("Sao chép thành công.")

        # Kết nối đến file database ở vị trí làm việc (trong AppData)
        self.conn = sqlite3.connect(self.db_path)
        
        # create_table được gọi để đảm bảo bảng luôn tồn tại
        self.create_table()
